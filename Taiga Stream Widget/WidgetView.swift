...
public class StreamInfo: NSObject, ObservableObject {
	...
	func startIcyPolling(streamUrl: URL) {
			...
			host.contains("somafm"), !streamUrl.pathComponents.isEmpty
		{
			...
			if !channel.isEmpty,
				let somaApi = URL(
					string: "https://somafm.com/songs/\(channel).json"
				)
			{
		...
		if let apiUrlString = apiUrlString,
	...
	private func pollAbcRadioMetadata(apiUrl: URL) {
		URLSession.shared.dataTask(with: apiUrl) {
			...
			if let artists = recording["artists"] as? [[String: Any]] {
				artist = artists.compactMap { $0["name"] as? String }.joined(
					separator: ", "
				)
			}
	...
	private func pollRadioFranceMetadata(apiUrl: URL) {
		...
		URLSession.shared.dataTask(with: request) {
			...
			if let delay = json["delayToRefresh"] as? TimeInterval {
				...
				DispatchQueue.main.async {
					self.icyPollTimer = Timer.publish(
					...
					.sink {
						[weak self] _ in
						self?.pollRadioFranceMetadata(apiUrl: apiUrl)
					}
				}
			}
			...
			if parts.count >= 2 {
				...
				title = self.cleanMetadataString(
			...
			self.lastIcyTitle = combined
			let resolvedArtist =
				artist.isEmpty || self.junkMetadata(artist)
				? "Taiga Stream" : artist
			let resolvedTitle =
				title.isEmpty ? "Stream \(self.currentStream)" : title
	...
	private func pollSomaFmMetadata(apiUrl: URL) {
		URLSession.shared.dataTask(with: apiUrl) {
			...
			let title = (first["title"] as? String ?? "").trimmingCharacters(
				in: .whitespaces
			)
			let artist = (first["artist"] as? String ?? "").trimmingCharacters(
				in: .whitespaces
			)
			let combined = "\(artist)|\(title)"
			guard !title.isEmpty, combined != self.lastIcyTitle else {
	...
	private func pollIcyMetadata(statusUrl: URL, mountPath: String) {
		URLSession.shared.dataTask(with: statusUrl) {
			...
			guard let source = match, let rawTitle = source["title"] as? String
			else {
				return
			}
				...
				let rawTitle = source["title"] as? String
			{
				...
				let songTitle = cleanMetadataString(
					rawTitle.trimmingCharacters(in: .whitespaces)
				)
				guard !songTitle.isEmpty, songTitle != self.lastIcyTitle else {
					return
				}

				self.lastIcyTitle = songTitle
				let resolvedArtist = artist.isEmpty ? "Taiga Stream" : artist
				let resolvedTitle =
					songTitle.isEmpty
					? "Stream \(self.currentStream)" : songTitle
				DispatchQueue.main.async {
					self.updateNowPlaying(
						artist: resolvedArtist,
						title: resolvedTitle
			...
			guard !title.isEmpty, title != self.lastIcyTitle else {
				return
			}
	...
	private func junkMetadata(_ value: String) -> Bool {
		...
		let adMarkerPattern =
			#"(?i)(spot\s+block|ad\s+break|commercial\s+break)"#

		if trimmedValue.range(of: adMarkerPattern, options: .regularExpression)
			!= nil
		{
			return true
		}
	...
	private func sessionObservers() {
		...
		.sink {
			...
			guard let streamUserInfo = streamNotification.userInfo,
				let streamTypeValue = streamUserInfo[
					AVAudioSessionInterruptionTypeKey
				] as? UInt,
				let streamType = AVAudioSession.InterruptionType(
					rawValue: streamTypeValue
				)
			else {
				return
			}
		...
		.sink {
			...
			guard let streamUserInfo = streamNotification.userInfo,
				...
				let streamType = AVAudioSession.SilenceSecondaryAudioHintType(
					rawValue: streamTypeValue
				)
			else {
				return
		...
		.sink {
			...
			guard let streamUserInfo = streamNotification.userInfo,
				...
				let streamType = AVAudioSession.RouteChangeReason(
					rawValue: streamTypeValue
				)
			else {
				return
	...
	func startPlaybackHeartbeat() {
		playbackHeartbeat = Timer.publish(every: 1.0, on: .main, in: .common)
			...
			.sink {
				...
				let actuallyPlaying =
					self.audioPlayer.timeControlStatus == .playing
